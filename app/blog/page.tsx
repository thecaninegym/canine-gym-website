    ...
    <div style={{ fontFamily: "'Segoe UI', sans-serif", color: '#111', backgroundColor: 'white' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #003087 0%, #004db3 100%)', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <p style={{ color: '#FF6B35', fontWeight: '700', fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', margin: '0 0 16px 0' }}>Resources</p>
      ...
      <section style={{ padding: '64px 24px 0', backgroundColor: 'white' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <p style={{ color: '#FF6B35', fontWeight: '700', fontSize: '12px', letterSpacing: '2px', textTransform: 'uppercase', margin: '0 0 20px 0' }}>Featured</p>
          <div style={{ backgroundColor: '#f0f4ff', borderRadius: '20px', padding: '48px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', alignItems: 'center', marginBottom: '64px' }}>
            <div>
              <span style={{ backgroundColor: '#003087', color: 'white', fontSize: '12px', fontWeight: '700', padding: '4px 12px', borderRadius: '20px', display: 'inline-block', marginBottom: '16px' }}>{posts[0].category}</span>
              <h2 style={{ fontSize: 'clamp(20px, 2.5vw, 28px)', fontWeight: '800', color: '#003087', margin: '0 0 16px 0', lineHeight: 1.3 }}>{posts[0].title}</h2>
            ...
            <div style={{ backgroundColor: '#d0daf5', borderRadius: '16px', height: '260px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '10px' }}>
              <PawPrint size={40} color="#003087" style={{ opacity: 0.3 }} />
      ...
      <section style={{ padding: '0 24px 80px', backgroundColor: 'white' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '36px' }}>
            {categories.map(cat => (
              <div key={cat} style={{ padding: '8px 18px', borderRadius: '20px', backgroundColor: cat === 'All' ? '#003087' : '#f0f4ff', color: cat === 'All' ? 'white' : '#003087', fontSize: '13px', fontWeight: '600', cursor: 'pointer', border: '1px solid ' + (cat === 'All' ? '#003087' : '#d0daf5') }}>
                {cat}
              </div>
            ))}
          </div>
            ...
            {posts.slice(1).map((post) => (
              <a key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: 'none', color: 'inherit', backgroundColor: '#f8f9fa', borderRadius: '16px', overflow: 'hidden', border: '1px solid #eee', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <span style={{ backgroundColor: '#f0f4ff', color: '#003087', fontSize: '11px', fontWeight: '700', padding: '3px 10px', borderRadius: '20px', display: 'inline-block', marginBottom: '12px', width: 'fit-content' }}>{post.category}</span>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ color: '#FF6B35', fontWeight: '700', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '4px' }}>Read <ArrowRight size={13} /></span>
                  </div>
                </div>
              </a>
            ))}
      ...
      <section style={{ padding: '80px 24px', backgroundColor: '#f8f9fa', textAlign: 'center' }}>
        <div style={{ maxWidth: '560px', margin: '0 auto' }}>
          <p style={{ color: '#FF6B35', fontWeight: '700', fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', margin: '0 0 12px 0' }}>Stay Updated</p>
          <h2 style={{ fontSize: '28px', fontWeight: '800', color: '#003087', margin: '0 0 12px 0' }}>Dog Health Tips, Monthly</h2>
          ...
          <div style={{ display: 'flex', gap: '10px', maxWidth: '440px', margin: '0 auto' }}>
            ...
            <button style={{ backgroundColor: '#FF6B35', color: 'white', padding: '13px 20px', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', whiteSpace: 'nowrap' }}>Subscribe</button>
          </div>
        </div>
      </section>
