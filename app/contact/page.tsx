    ...
    <div style={{ fontFamily: "'Montserrat', sans-serif", color: '#111', backgroundColor: 'white' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #2c5a9e 0%, #3d70c0 100%)', padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
          <p style={{ color: '#f88124', fontWeight: '700', fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', margin: '0 0 16px 0' }}>Get In Touch</p>
...
<div className="contact-grid">
          ...
          <div>
            ...
            {[
              ...
              { icon: <MapPin size={20} color="#f88124" />, label: 'Service Area', value: 'Hamilton County, Indiana', href: null },
              { icon: <Clock size={20} color="#f88124" />, label: 'Response Time', value: 'Within 1 business day', href: null },
            ].map((item, i) => (
              <div key={i} style={{ display: 'flex', gap: '14px', marginBottom: '24px', alignItems: 'flex-start' }}>
                <div style={{ width: '40px', height: '40px', backgroundColor: '#fff4e6', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {item.icon}
                </div>
                <div>
                  <p style={{ color: '#999', fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px', margin: '0 0 4px 0' }}>{item.label}</p>
                  {item.href ? (
                    <a href={item.href} style={{ color: '#2c5a9e', fontWeight: '600', fontSize: '15px', textDecoration: 'none' }}>{item.value}</a>
                  ) : (
            ...
            <div style={{ backgroundColor: '#eef2fb', borderRadius: '14px', padding: '20px', marginTop: '32px', border: '1px solid #c8d4f0' }}>
              <p style={{ fontWeight: '700', color: '#2c5a9e', margin: '0 0 6px 0', fontSize: '14px' }}>Already a customer?</p>
              ...
              <a href="https://app.thecaninegym.com" style={{ color: '#f88124', fontWeight: '700', fontSize: '14px', textDecoration: 'none' }}>Go to App →</a>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '40px', border: '1px solid #eee', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
            {submitted ? (
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                <PawPrint size={48} color="#f88124" style={{ marginBottom: '16px' }} />
                <h3 style={{ fontWeight: '800', fontSize: '22px', color: '#2c5a9e', margin: '0 0 12px 0' }}>Message Sent!</h3>
                <p style={{ color: '#666', fontSize: '15px', lineHeight: 1.6 }}>Thanks for reaching out. We'll get back to you within one business day.</p>
              </div>
            ) : (
              <>
                <h3 style={{ fontWeight: '800', fontSize: '20px', color: '#2c5a9e', margin: '0 0 28px 0' }}>Send Us a Message</h3>
                <form onSubmit={handleSubmit}>
                  ...
                  <button type="submit" disabled={loading} style={{ width: '100%', backgroundColor: loading ? '#ccc' : '#f88124', color: 'white', padding: '14px', borderRadius: '8px', border: 'none', fontWeight: 'bold', fontSize: '16px', cursor: loading ? 'not-allowed' : 'pointer' }}>
