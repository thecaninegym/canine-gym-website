...
import { PawPrint, MapPin, Calendar, Clock, Shield, BarChart2, Heart, Trophy } from 'lucide-react'

export const metadata: Metadata = {
  title: 'Dog Fitness in Whitestown, IN | Mobile Slatmill Sessions | The Canine Gym',
  description: 'Mobile dog gym sessions in Whitestown, Indiana. The Canine Gym brings professional slatmill workouts to your home. 30-minute canine fitness sessions. Book today.',
  alternates: {
    canonical: 'https://www.thecaninegym.com/whitestown',
  },
  openGraph: {
    url: 'https://www.thecaninegym.com/whitestown',
    title: 'Dog Fitness in Whitestown, IN | The Canine Gym',
  },
}

export default function Whitestown() {
  return (
    <div style={{ fontFamily: "'Montserrat', sans-serif", color: '#111', backgroundColor: 'white' }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #2c5a9e 0%, #3d70c0 100%)', padding: '80px 24px 100px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '720px', margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: 'rgba(248,129,36,0.2)', border: '1px solid rgba(248,129,36,0.4)', borderRadius: '20px', padding: '6px 16px', marginBottom: '28px' }}>
            ...
            <span style={{ color: '#f88124', fontSize: '13px', fontWeight: '600' }}>Now serving Whitestown, Indiana</span>
          </div>
          <h1 style={{ color: 'white', fontSize: 'clamp(34px, 5.5vw, 60px)', fontWeight: '800', margin: '0 0 20px 0', lineHeight: 1.1, letterSpacing: '-1px' }}>
            Professional Dog Fitness<br /><span style={{ color: '#f88124' }}>In Whitestown.</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 'clamp(16px, 2.5vw, 19px)', lineHeight: 1.6, margin: '0 0 40px 0', maxWidth: '560px', marginLeft: 'auto', marginRight: 'auto' }}>
            Whitestown is a fast-growing community in Boone County, just southwest of our core service area. We bring professional slatmill sessions directly to your door twice a week.
          </p>
          <a href="https://app.thecaninegym.com" className="btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', backgroundColor: '#f88124', color: 'white', padding: '16px 36px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '17px' }}>
            <PawPrint size={20} /> Book a Session in Whitestown
          </a>
        </div>
      </section>
      ...
      <section style={{ padding: '72px 24px', backgroundColor: 'white' }}>
        <div style={{ maxWidth: '700px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(26px, 3.5vw, 38px)', fontWeight: '800', color: '#2c5a9e', margin: '0 0 16px 0', letterSpacing: '-0.5px' }}>When We're in Whitestown</h2>
          <p style={{ color: '#666', fontSize: '16px', margin: '0 0 40px 0', lineHeight: 1.6 }}>We're in Whitestown on Mondays and Thursdays. Pick the time that works best for your dog.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', maxWidth: '440px', margin: '0 auto 36px' }}>
            {[
              { day: 'Monday', time: '1:00 PM – 7:00 PM' },
              { day: 'Thursday', time: '6:00 AM – 12:00 PM' },
            ].map((slot, i) => (
      ...
      <section style={{ padding: '72px 24px', backgroundColor: '#f8f9fa' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <p style={{ color: '#f88124', fontWeight: '700', fontSize: '13px', letterSpacing: '2px', textTransform: 'uppercase', margin: '0 0 12px 0' }}>Why Slatmill</p>
            <h2 style={{ fontSize: 'clamp(26px, 3.5vw, 38px)', fontWeight: '800', color: '#2c5a9e', margin: '0 0 16px 0', letterSpacing: '-0.5px' }}>Better Than the Trail. Better Than the Park.</h2>
            <p style={{ color: '#666', fontSize: '16px', maxWidth: '580px', margin: '0 auto', lineHeight: 1.6 }}>
              Whitestown has wonderful trails and parks. But for consistent, measurable, structured fitness, nothing beats a supervised slatmill session.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px' }}>
            {[
              { icon: <Heart size={24} color="#f88124" />, title: 'Consistent Cardio', desc: 'Rain, heat, or cold, your dog\'s workout happens regardless of what\'s happening outside.' },
              { icon: <Shield size={24} color="#f88124" />, title: 'No Distractions', desc: 'No other dogs, no squirrels, no hazards. Just a focused, productive session every time.' },
              { icon: <BarChart2 size={24} color="#f88124" />, title: 'Measurable Progress', desc: 'Every metric tracked and saved so you can actually see your dog improving over time.' },
              { icon: <Trophy size={24} color="#f88124" />, title: 'Earn Achievements', desc: 'Your dog earns badges and climbs the city leaderboard as their fitness grows.' },
            ].map((item, i) => (
      ...
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, #2c5a9e 0%, #3d70c0 100%)', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <h2 style={{ color: 'white', fontSize: 'clamp(26px, 4vw, 40px)', fontWeight: '800', margin: '0 0 16px 0', letterSpacing: '-0.5px' }}>Whitestown Dogs Run With Us.</h2>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '17px', margin: '0 0 36px 0', lineHeight: 1.6 }}>Sign up today and get your first session on the books. We come to you.</p>
          <a href="https://app.thecaninegym.com" className="btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', backgroundColor: '#f88124', color: 'white', padding: '16px 36px', borderRadius: '10px', textDecoration: 'none', fontWeight: 'bold', fontSize: '17px' }}>
            <PawPrint size={20} /> Book in Whitestown
          </a>
        </div>
      </section>
